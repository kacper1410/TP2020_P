...
namespace Task3
{
    public class Class1
    {
        public void method()
        {
            ProductionDataContext context = new ProductionDataContext();
            IEnumerable<string> names = from product in context.Products
                                        where product.ProductNumber == "112"
                                        select product.Name;

            //IEnumerable<string> names = context.Products.Where(p => p.ProductNumber == "112").Select(p => p.Name);
